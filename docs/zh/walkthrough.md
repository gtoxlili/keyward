...
# SINGLE_TENANT=1：单租户模式，只一个 Client、不需要路由 token。
...
现在像一个无感知的 OpenAI 应用那样，通过 Node 发起调用——它的 base URL 就是这个 Node。单租户模式下随便什么
bearer 都会路由到你这唯一一个已配对的 Client；若是多租户节点，这里就必须用该 Client 的路由 token：
