...
不能，永远不能。它发出去的只是工作意图；key 是在你的 Client 内部才补上的。这正是你可以亲自验证的那个承诺
...
放在 Client 上，而且每次调用前都会执行一遍。

**这不就是个代理 / LiteLLM 吗？**

不是。网关是*持有*你的 key、替你转发请求——那是托管式的，你又回到了「信任一台服务器」。而 Keyward 的
Node 什么都不持有，只要你这边没有一个运行中的 Client，它就根本发不出任何调用。

**我关掉标签页 / 停掉 Client 会怎样？**

所有活儿立刻全停——没有运行中的 Client，Node 就调不动 Provider。要跑自治任务，就把 Client
常驻在你自己的机器上。
