//! End-to-end seal layer for the trustless-node case (§9/§10).
//!
//! The requester (shim) seals each request to the Client's **identity** key; the node
//! relays only ciphertext; the Client decrypts, runs, and seals the reply back. The
//! node never sees the prompt — only the routing token (in the envelope) and opaque blobs.
//!
//! Profile: X25519 ECDH between a fresh ephemeral (requester) and the Client's static
...
//! relayed through the node, but gives no per-message forward secrecy (compromising the
//! Client's long-term key later exposes recorded traffic). Interactive Noise XX, with FS,
...
/// Client X25519 secret from its Ed25519 signing key
...
/// Client X25519 public from its Ed25519 identity public (Montgomery u-coordinate) —
/// what the shim derives from the Client's known identity pubkey.
...
impl Sealed {
    ...
    /// hand the Client (hex).
    pub fn initiator(client_pub: &PublicKey) -> (Self, String) {
        let eph = SecretKey::generate(&mut OsRng);
        let epk = hex(&eph.public_key().to_bytes());
        (
            Self {
                cbox: ChaChaBox::new(client_pub, &eph),
            },
            epk,
        )
    }

    /// Client side: reconstruct the channel from its static secret + the ephemeral pubkey.
    pub fn responder(client_secret: &SecretKey, eph_pub_hex: &str) -> Result<Self> {
        ...
        Ok(Self {
            cbox: ChaChaBox::new(&PublicKey::from(epk), client_secret),
        })
    }
...
mod tests {
    ...
    fn ed_to_x25519_public_agrees() {
        ...
        // Client's secret derives — else ECDH wouldn't agree.
    ...
    fn seal_round_trips_and_wrong_key_fails() {
        let client = SigningKey::generate(&mut rand_core::OsRng);
        let client_pub = x25519_public(&client.verifying_key());

        // shim seals a request to the client's identity pubkey
        let (shim, epk) = Sealed::initiator(&client_pub);
        let req = shim.seal(br#"{"prompt":"secret"}"#).unwrap();

        // client reconstructs from its secret + the ephemeral pubkey, decrypts
        let client_chan = Sealed::responder(&x25519_secret(&client), &epk).unwrap();
        assert_eq!(client_chan.open(&req).unwrap(), br#"{"prompt":"secret"}"#);

        // client seals a reply; the shim opens it
        let reply = client_chan.seal(b"pong").unwrap();
        assert_eq!(shim.open(&reply).unwrap(), b"pong");

        // a different key (a curious node) cannot open the request
